...
(function () {
  ...
  const win: any = hasWindow ? (globalThis as any).window : undefined;

  const isContentScript =
    hasWindow && typeof win.wrappedJSObject !== "undefined";

  console.log(
    "WASM hook running in:",
    isWorker
      ? "WORKER"
      : isContentScript
      ? "CONTENT SCRIPT"
  ...
  // Choose correct global
  const pageWindow: any = isWorker
    ? globalThis
    : isContentScript
    ? win.wrappedJSObject
    : win;

  // exportFunction only exists in Firefox content scripts
  const exportToPage =
    isContentScript && typeof win.exportFunction === "function"
      ? (fn: Function) => win.exportFunction(fn, pageWindow)
      : (fn: Function) => fn;


    (async function () {
    // Check if the WebAssembly hook has already been injected.
    if (Object.prototype.hasOwnProperty.call(pageWindow.WebAssembly, "__hooked__")) {
    ...
    if (
      "serviceWorker" in pageWindow.navigator &&
      ...
      try {
        const registrations = await pageWindow.navigator.serviceWorker.getRegistrations();
    ...
    const originalCryptoSubtle: SubtleCrypto = pageWindow.crypto.subtle;

    // Mark WebAssembly as hooked.
    Object.defineProperty(pageWindow.WebAssembly, "__hooked__", {
      value: true,
      writable: false,
      configurable: false,
      enumerable: false,
    ...
    function extractBuffer(bufferSource: BufferSource | WebAssembly.Module): ArrayBuffer {
    ...
    //
    const originalInstantiate = pageWindow.WebAssembly.instantiate;
    ...
    async function hookedInstantiate(
      ...
      if (source instanceof pageWindow.WebAssembly.Module) {
        return originalInstantiate.call(this, source, importObject);
      } else {
        // If source is a Promise, await it.
        const sourceBuffer: WebAssembly.Module | BufferSource | Promise<BufferSource> =
    ...
    pageWindow.WebAssembly.instantiate = exportToPage(hookedInstantiate) as typeof WebAssembly.instantiate;

    //
    // Hook WebAssembly.compile (async)
    //
    const originalCompile = pageWindow.WebAssembly.compile;
    pageWindow.WebAssembly.compile = exportToPage(async function (
      this: typeof pageWindow.WebAssembly,
    ...
    //
    const originalValidate = pageWindow.WebAssembly.validate;
    pageWindow.WebAssembly.validate = exportToPage(function (this: typeof pageWindow.WebAssembly,bufferSource: BufferSource): boolean {
      const buffer: ArrayBuffer = extractBuffer(bufferSource);
    ...
    //
    const originalInstantiateStreaming = pageWindow.WebAssembly.instantiateStreaming;
    pageWindow.WebAssembly.instantiateStreaming = exportToPage(async function (
      this: typeof pageWindow.WebAssembly,
    ...
    //
    const originalCompileStreaming = pageWindow.WebAssembly.compileStreaming;
    pageWindow.WebAssembly.compileStreaming = exportToPage(async function (
      this: typeof pageWindow.WebAssembly,
      responseOrPromise: Response | PromiseLike<Response>,
    ): Promise<WebAssembly.Module> {
      const response: Response = await Promise.resolve(responseOrPromise);
    ...
    // Hook the WebAssembly.Module constructor (synchronous)
    const OriginalModule = pageWindow.WebAssembly.Module;
    ...
    pageWindow.WebAssembly.Module = exportToPage(hookedModule) as typeof pageWindow.WebAssembly.Module;

    // Use the universal global object.
    const globalObj = pageWindow;
