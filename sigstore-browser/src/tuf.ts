...
async function fetchMetafile(role: string, version: number|string = -1, json: boolean = true): Promise<any> {
    var url: string = "";
    if (version != -1) {
...
async function updateTargets(root: Root, frozenTimestamp: Date, snapshot: Meta) {
    ...
    if (root.consistent_snapshot) {
        newTargetsRaw = await fetchMetafile(Roles.Targets, snapshot[`${Roles.Targets}.json`].version, false);
    } else {
...
export async function fetchSigstoreTrustRoot() {
    ...
    browser.storage.local.set({[Roles.TrustedRoot]: JSON.parse(Uint8ArrayToString(trusted_root_raw))});
}
